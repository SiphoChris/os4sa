/**
 * sections/index.tsx
 * Every major page section as a named export.
 * Sections are purely presentational — data and state live in data/ and App.tsx.
 */
...
import { Badge, BentoCard, GridWrapper, SectionHeader, StripeAccent } from '../ui-primitives';
import { ProjectCard } from '../ProjectCard';
import { NAV_LINKS, PROJECTS, GAPS, REALITIES, PRINCIPLES } from '../../data';
import type { GapItem, RealityItem } from '../../types';
        ...
        <div className="md:hidden">
          <Sheet open={isMenuOpen} onOpenChange={setIsMenuOpen}>
            ...
            <SheetContent
              ...
              className="bg-os4sa-bg border-white/10 text-white p-0 overflow-hidden flex flex-col"
            >
              ...
              <div className="flex-1 flex flex-col p-8 pt-4 relative z-10">
                ...
                <div className="mt-12 space-y-6">
                  ...
                  <div className="flex items-center justify-center gap-8 pt-4">
                    <a href="https://github.com/os4sa" className="text-white/40 hover:text-white transition-colors"><Github className="w-6 h-6" /></a>
                    <a href="#"                         className="text-white/40 hover:text-white transition-colors"><Twitter className="w-6 h-6" /></a>
                    <a href="#"                         className="text-white/40 hover:text-white transition-colors"><Mail className="w-6 h-6" /></a>
                  </div>
                </div>
    ...
    <section className="relative min-h-[100dvh] flex flex-col justify-center items-center px-6 pt-20 text-center overflow-hidden border-b border-grid">
      ...
      >
        ...
        <h1 className="mt-8 text-5xl sm:text-6xl md:text-7xl lg:text-9xl font-display font-bold tracking-tighter leading-[0.9] md:leading-[0.85]">
          ...
          <br />
          <span className="text-os4sa-gold">
            the Public <br className="sm:hidden" /> Good.
          </span>
        </h1>
        ...
        <div className="mt-10 md:mt-12 flex flex-col sm:flex-row justify-center gap-4 px-4">
          <Button
            asChild
            size="lg"
            className="bg-white text-os4sa-bg hover:bg-os4sa-gold rounded-full px-8 py-6 md:py-7 font-bold uppercase tracking-widest text-[10px] md:text-xs transition-all hover:scale-105 active:scale-95 w-full sm:w-auto"
          >
          ...
          <Button
            ...
            variant="outline"
            size="lg"
            className="glass border-white/10 hover:bg-white/10 rounded-full px-8 py-6 md:py-7 font-bold uppercase tracking-widest text-[10px] md:text-xs transition-all w-full sm:w-auto"
          >
  ...
  <section id="projects" className="py-20 md:py-32 px-4 md:px-12 border-b border-grid">
    <GridWrapper>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 mb-12 md:mb-20 px-4 md:px-8">
        ...
        <Button
          variant="outline"
          className="glass border-white/10 hover:bg-white/10 rounded-full px-6 py-6 flex items-center gap-2 text-white/80 font-mono uppercase tracking-widest text-[10px] transition-all mb-12 md:mb-20 w-full md:w-auto justify-center"
        >
...
// ─── BentoGridSection ─────────────────────────────────────────────────────────
// Generic bento layout for Gaps and Realities sections.

interface BentoGridSectionProps {
  badge: string;
  title: string;
  subtitle: string;
  items: (GapItem | RealityItem)[];
  id?: string;
}

export const BentoGridSection = ({ badge, title, subtitle, items, id }: BentoGridSectionProps) => (
  <section id={id} className="py-20 md:py-32 px-4 md:px-12 border-b border-grid">
    <GridWrapper>
      <div className="px-4 md:px-8">
        <SectionHeader badge={badge} title={title} subtitle={subtitle} centered />

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4 md:gap-6 pb-12 md:pb-20">
          {items.map((item, i) => {
            const size = (item as GapItem).size;
            return (
              <BentoCard
                key={i}
                delay={i * 0.05}
                ...
                className={cn(
                  ...
                  size === 'full'  ? 'lg:col-span-6 md:col-span-2' :
                  size === 'large' ? 'lg:col-span-4 md:col-span-2' :
    ...
    <section id="guide" className="py-20 md:py-32 px-6 md:px-12 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center relative z-10">
        ...
        >
          ...
          <h2 className="mt-4 text-3xl md:text-6xl font-display font-bold tracking-tight mb-6 md:mb-8">
            The Practitioner's <br /> Guide
          </h2>
          ...
          <Button
            asChild
            size="lg"
            className="bg-white text-os4sa-bg hover:bg-os4sa-gold rounded-full px-10 py-6 md:py-8 font-bold uppercase tracking-widest text-[10px] md:text-xs transition-all flex items-center gap-3 mx-auto lg:mx-0"
          >
            <a
          ...
          <div className="relative glass p-2 rounded-3xl shadow-2xl">
            <div className="bg-os4sa-bg p-8 md:p-16 aspect-[3/4] flex flex-col justify-between border border-white/5 rounded-2xl overflow-hidden relative">
              ...
              <div>
                ...
                <p className="text-[8px] md:text-[10px] font-mono text-white/30 uppercase tracking-[0.5em]">
                  Practitioner's Guide v1.0
                </p>
              </div>
              <div className="space-y-4 md:space-y-6">
                {[null, null, null, null].map((_, i) => (
                  <div key={i} className={`h-1 md:h-1.5 ${i % 2 === 1 ? 'w-4/5' : i === 3 ? 'w-3/4' : 'w-full'} bg-white/5 rounded-full`} />
                ))}
              </div>
...
export const Principles = () => (
  <section
    id="principles"
    className="py-20 md:py-32 px-6 md:px-12 bg-white text-os4sa-bg rounded-[32px] md:rounded-[80px] mx-4 md:mx-8 my-12"
  >
    <div className="max-w-7xl mx-auto">
      <div className="mb-16 md:mb-24 text-center md:text-left">
        ...
        <p className="mt-4 text-os4sa-bg/60 text-base md:text-lg max-w-2xl mx-auto md:mx-0">
          Distilled from the realities of building for South African government.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-12 md:gap-x-16 md:gap-y-16">
        {PRINCIPLES.map((p, i) => (
          ...
          >
            <span className="font-mono text-os4sa-gold font-bold text-2xl md:text-3xl opacity-20 group-hover:opacity-100 transition-opacity">
...
export const CTA = () => (
  <section className="py-24 md:py-40 px-6 md:px-12 relative overflow-hidden">
    ...
    <div className="max-w-4xl mx-auto text-center relative z-10">
      ...
      <Button
        size="lg"
        className="bg-white text-os4sa-bg hover:bg-os4sa-gold rounded-full px-10 py-6 md:py-8 font-bold uppercase tracking-widest text-[10px] md:text-xs transition-all hover:scale-105 active:scale-95 flex items-center gap-3 mx-auto w-full sm:w-auto justify-center"
      >
        Join the Ecosystem
        <ChevronRight className="w-4 h-4" />
      </Button>
      ...
      <div className="flex flex-col">
        ...
        <div className="flex flex-col gap-4 text-xs font-mono uppercase tracking-widest text-white/60">
          ...
          <p className="text-[9px] leading-relaxed opacity-50">
            Not affiliated with DPSA, SITA, or any government department.
          </p>
        </div>
      </div>
